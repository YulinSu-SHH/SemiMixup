...
parser.add_argument('--train', type=str, default = 'vanilla', choices =['vanilla','mixup', 'mixup_hidden','mixup_stages','mixup_hidden_stages'])
...
os.environ["CUDA_VISIBLE_DEVICES"] = "1"#str(args.device)
...
def train(train_loader,model,optimizer, epoch, args, log):
    ...
    for i, (input_, target) in enumerate(train_loader):
        ...
        if args.attack or args.partial_attack:
        ...
        ###  supervised training  ####
        if args.attack:
            input_var, target_var = Variable(input_attack), Variable(target)
        elif args.partial_attack:
        ...
        if args.train == 'vanilla':
            ...
            assert False
        # if epoch < len_sup//args.batch_size:
        loss = bce_loss(softmax(output), reweighted_target)
        # else:
        #     loss = inception_score(output, splits=10)
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #else:
            #tr_acc, tr_acc5, tr_los = train(all_train_loader,len(labeled_train_loader),net, optimizer, epoch, args, log)
        ...
        #     tr_acc, tr_acc5, tr_los = train(pseudo_loader, len(pseudo_loader),net, optimizer, epoch, args, log)
        #     train_loss.append(tr_los)
        #     train_acc.append(tr_acc)
