...
parser.add_argument('--train', type=str, default = 'mixup_hidden', choices =['vanilla','mixup', 'mixup_hidden','mixup_stages','mixup_hidden_stages'])
...
def train(train_loader,model,optimizer, epoch, args, log,uda=False):
    ...
    for i, (input_, target) in enumerate(train_loader):
        ...
        if uda:
            loss = inception_score(softmax(output), splits=10)
        else:
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.uda_loss:
            ...
            if val_acc/100.>thre:
                tr2_acc, tr2_acc5, tr2_los = train(unlabeled_train_loader, net, optimizer, epoch, args, log,uda=True)
