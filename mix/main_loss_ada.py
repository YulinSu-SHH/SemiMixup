...
parser.add_argument('--labels_per_class', type=int, default=500, metavar='NL',
                    help='labels_per_class')
parser.add_argument('--unlabels_per_class', type=int, default=1000, metavar='NL',
...
parser.add_argument('--train', type=str, default = 'mixup_hidden', choices =['mixup_hidden','stages_mixup_hidden'])
parser.add_argument('--uda_loss',action='store_true',default=True,help='inception score loss')
#parser.add_argument('--pseudo',action='store_true',default=False,help='self training')
parser.add_argument('--mixup_alpha', type=float, default=2.0, help='alpha parameter for mixup')
...
parser.add_argument('--schedule', type=int, nargs='+', default=[500, 1000,1500], help='Decrease learning rate at these epochs.')
...
def experiment_name_non_mnist(dataset='cifar10',arch='preresnet18',train = 'vanilla_pseudo'):
   ...
   # if args.pseudo :
   #     exp_name += '_pseudo'
    exp_name += '_thre_uncertainty_estimation'
...
def train_standard(train_loader,model,optimizer, epoch, args,log):
...
def train_is_loss(train_loader,model,optimizer, epoch, args,log):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    ...
    for i, (input_, target) in enumerate(train_loader):
        ...
        if args.train == 'mixup_hidden':
            output1,output, reweighted_target = model(x=input_var, target=target_var, mixup_hidden=True,mix=True,
                                              mixup_alpha=args.mixup_alpha, noise=args.noise)


        elif args.train == 'stages_mixup_hidden':
            ...
            assert False
        var = _kl_divergence_with_logits(softmax(output1), softmax(output))
        exp_var = torch.exp(-var)
        loss = bce_loss(softmax(output), reweighted_target)
        loss = torch.mean(loss * exp_var) + torch.mean(var)
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        #adv_test_dataset=attack_test_data(test_data,net,batch_size=128,num_iter=args.pgd_step_size,eps=args.pgd_eps,alpha=args.pgd_alpha)
        ...
        print_log(
            '\n==>>{:s} [Epoch={:03d}/{:03d}] {:s} [learning_rate={:6.4f}]  Best : Test Accuracy={:.2f}'.format(time_string(), epoch, args.epochs,
                           need_time, current_learning_rate,recorder.max_accuracy(False)),log)# \
       #     + (' [Best : Test Accuracy={:.2f}, 'format(recorder.max_accuracy(False))), log)
              # 'Adv Test Accuracy={:.2f}]'.format(recorder.max_accuracy(False),recorder.max_at_aft_acc())), log)

        # train for one epoch
        tr_acc, tr_acc5, tr_los = train_standard(labeled_train_loader, net, optimizer, epoch, args, log)
        val_acc, val_los = validate(epoch, test_loader, net, "Test", log)
        #pseudo_dataset = pseudo_data(unlabeled_train_loader, net, args.noise, 0.)
        #pseudo_loader = torch.utils.data.DataLoader(pseudo_dataset, batch_size=args.batch_size, shuffle=True,
                                                    #num_workers=args.workers, pin_memory=True)
        tr2_acc, tr2_acc5, tr2_los = train_is_loss(unlabeled_train_loader, net, optimizer, epoch, args, log)
