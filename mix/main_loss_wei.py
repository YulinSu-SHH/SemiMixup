...
parser.add_argument('--train', type=str, default = 'vanilla_pseudo', choices =['mixup_hidden','stages_mixup_hidden'])
...
def train_is_loss(train_loader,model,optimizer, epoch, args,log,acc):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    ...
    for i, (input_, target) in enumerate(train_loader):
        ...
        weight = np.exp(acc-1)
        loss_ = inception_score(softmax(output), splits=10)
     ...
     #   adv_test_dataset.inputs = adv_test_dataset.inputs.cpu()
      ...
      #  adv_test_loader = torch.utils.data.DataLoader(adv_test_dataset, batch_size=args.batch_size, shuffle=False,
       #                                          num_workers=args.workers, pin_memory=True)
        ...
        tr2_acc, tr2_acc5, tr2_los = train_is_loss(unlabeled_train_loader, net, optimizer, epoch, args, log,val_acc/100.)
