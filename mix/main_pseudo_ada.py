...
parser.add_argument('--train', type=str, default = 'vanilla_pseudo', choices =['mix_pseudo','partial_mix_pseudo','stages_mix_pseudo'])
...
def train_pseudo(train1_loader,train2_loader,model,optimizer, epoch, args,log):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    ...
    for i in range(len(train1_loader)):
        ...
        if args.train == 'mix_pseudo':
            output1,output, reweighted_target = model(x=input_var,target= target_var, mixup_hidden=True, mix=True,mixup_alpha=args.mixup_alpha,noise=args.noise)

        elif args.train == 'partial_mix_pseudo':
            output1,output, reweighted_target = model(x=input_var,xb=input_2_var,target= target_var, targetb=target_2_var,mixup_hidden=True, mix=True,mixup_alpha=args.mixup_alpha,noise=args.noise)

        elif args.train == 'stages_mix_pseudo':
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        pseudo_loader = torch.utils.data.DataLoader(pseudo_dataset, batch_size=args.batch_size, shuffle=True,
                                                    num_workers=args.workers, pin_memory=True)
        tr2_acc, tr2_acc5, tr2_los = train_pseudo(pseudo_loader, labeled_train_loader, net, optimizer, epoch, args, log)
