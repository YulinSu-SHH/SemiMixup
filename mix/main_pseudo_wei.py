...
def train_pseudo(train1_loader,train2_loader,model,optimizer, epoch, args,log,acc):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    ...
    for i in range(len(train1_loader)):
        ...
        weight = np.exp(acc-1)
        loss_ = bce_loss(softmax(output), reweighted_target)
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        pseudo_loader = torch.utils.data.DataLoader(pseudo_dataset, batch_size=args.batch_size, shuffle=True,
                                                    num_workers=args.workers, pin_memory=True)
        tr2_acc, tr2_acc5, tr2_los = train_pseudo(pseudo_loader, pseudo_loader, net, optimizer, epoch, args, log,val_acc/100.)
