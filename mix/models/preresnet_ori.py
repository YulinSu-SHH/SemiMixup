...
class PreActResNet(nn.Module):
    ...
    def forward(self, x, xb=None, target= None, targetb=None,mixup=False, mixup_hidden=False, mixup_alpha=None, noise=0.0):
        if self.per_img_std:
        ...
        if targetb is None and target is not None :
            targetb=target
        ...
        if targetb is not None:
            targetb_reweighted = to_one_hot(targetb, self.num_classes, noise=noise)

        if xb is None:
            xb = x
        
        if layer_mix == 0:
            #print(out.size(),xb.size())
            out, target_reweighted = mixup_process(out, xb, target_reweighted,targetb_reweighted, lam=lam)
        else:
            xb = self.conv1(xb)
            xb = self.layer1(xb)
        ...
        if layer_mix == 1:
            out, target_reweighted = mixup_process(out, xb, target_reweighted, target_reweighted,lam=lam)

        if layer_mix != 0 and layer_mix != 1:
            xb = self.layer2(xb)
